...
import { getPostsByTag, getAllTags } from '@/app/blog/lib/posts'
...
export async function generateMetadata({ params }: { params: Promise<{ tag: string }> }) {
  ...
  return {
    title: `#${tagTitle} Articles | Next Wave Wellness Blog`,
    description: `Browse all articles tagged with ${tagTitle}. Expert insights and reviews from the Next Wave Wellness team.`,
  }
}
...
export default async function TagPage({ params }: { params: Promise<{ tag: string }> }) {
  const { tag } = await params
  const tagTitle = slugToTitle(tag)
  const posts = getPostsByTag(tagTitle)
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="pt-24 pb-4 px-6 max-w-7xl mx-auto">
        <nav className="text-sm text-gray-600">
          ...
          <span className="mx-2">/</span>
          <span className="text-gray-900">#{tagTitle}</span>
        </nav>
      </section>
      ...
      <section className="pb-16 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold mb-4">#{tagTitle}</h1>
